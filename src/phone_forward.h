 ...
 *
 * @author Marcin Peczarski <marpe@mimuw.edu.pl>, Krzysztof Hajderek <kh438485@mimuw.edu.pl>
...
/**
 * Numerem telefonu nazywamy niepusty napis złożony z cyfr i znaków *, #.
 */
 ...
 */
struct PhoneForward;
typedef struct PhoneForward PhoneForward; ///< krótszy zapis "struct PhoneForward"

/**
 * To jest struktura przechowująca ciąg numerów telefonów.
 */
struct PhoneNumbers;
typedef struct PhoneNumbers PhoneNumbers; ///< krótszy zapis "struct PhoneNumbers"
 ...
 *         reprezentuje numeru lub pf == NULL lub
 ...
 *         udało się alokować pamięci albo pf == NULL.
 */
...
/** @brief Wyznacza przekierowania na dany numer.
 * Wyznacza ciąg wszystkich numerów x, takich że dla pewnego prefiksu p napisu x
 * i pewnego napisu q zachodzą wszystkie trzy poniższe warunki:
 * (1) Została wywołana procedura @ref phfwdAdd z parametrami @p pf , p, q.
 * (2) Efekt powyższego nie został anulowany przy pomocy @ref phfwdRemove.
 * (3) Po zastąpieniu p przez q w napisie x otrzymamy @p num .
 * Dodatkowo ciąg wynikowy zawsze zawiera też numer @p num. Wynikowe numery są
 * posortowane leksykograficznie i nie mogą się powtarzać. Jeśli podany napis nie
 * reprezentuje numeru, wynikiem jest pusty ciąg. Alokuje strukturę
 * @p PhoneNumbers, która musi być zwolniona za pomocą funkcji @ref phnumDelete.
 * @param[in] pf  – wskaźnik na strukturę przechowującą przekierowania numerów;
 * @param[in] num – wskaźnik na napis reprezentujący numer.
